...
NUM_WORKERS = 0         # Keep 0 for safer execution in VS Code / SSH
...
# Target: first character of the ICD code.
# Do NOT lowercase, because Kaggle uses strict string comparison.
...
if class_counts.min() >= 2:
    ...
    print("Warning: at least one class has fewer than 2 examples. Stratified split disabled.")
...
save_csv_checked(
    history_df,
    OUTPUT_DIR / f"training_history_{POOLING}.csv"
)
...
if not checkpoint_path.exists():
    raise FileNotFoundError(
        ...
        f"Training may have stopped before saving the model."
    )
...
# 1. id must be the first column
# 2. Literal must be included
# 3. y_category must have no empty values
